...
void LO_FrameToFrame_PlaneSegmentation::processFrame(
 ...
 void LO_FrameToFrame_PlaneSegmentation::
                                 ...
                                 // reasonable
    {
        ...
        auto current_pcd_plane_down_fpfh = open3d::registration::ComputeFPFHFeature(*current_pcd_plane_down, open3d::geometry::KDTreeSearchParamHybrid(0.25, 100));

        auto reference_pcd_plane_down = reference_pcd_plane->VoxelDownSample(voxel_size);
        ...
        auto reference_pcd_plane_down_fpfh = open3d::registration::ComputeFPFHFeature(*reference_pcd_plane_down, open3d::geometry::KDTreeSearchParamHybrid(0.25, 100));

        /////////////////////////////////
        //std::shared_ptr<open3d::geometry::PointCloud> pointcloud_ptr_try0(new open3d::geometry::PointCloud);
        //open3d::geometry::PointCloud pointcloud_print = current_lidar_frame_->point_cloud_;
        //*pointcloud_ptr_try0 = pointcloud_print;
        ...
        /////////////////////////////////////////////////////////////////////////////////////
         auto current_pcd_without_plane_down = current_pcd_without_plane->VoxelDownSample(voxel_size);
        current_pcd_without_plane_down->EstimateNormals(
            open3d::geometry::KDTreeSearchParamHybrid(0.1, 30));
        current_pcd_without_plane_down->RemoveNonFinitePoints(remove_nan, remove_infinite);
        current_pcd_without_plane_down->RemoveRadiusOutliers(nb_points, search_radius);
        current_pcd_without_plane_down->RemoveStatisticalOutliers(nb_neighbors, std_ratio);
        
        auto current_pcd_without_plane_down_fpfh = open3d::registration::ComputeFPFHFeature(
            *current_pcd_without_plane_down, open3d::geometry::KDTreeSearchParamHybrid(0.4, 100));

        auto reference_pcd_without_plane_down = reference_pcd_without_plane->VoxelDownSample(voxel_size);
        reference_pcd_without_plane_down->EstimateNormals(
            open3d::geometry::KDTreeSearchParamHybrid(0.1, 30));
        reference_pcd_without_plane_down->RemoveNonFinitePoints(remove_nan, remove_infinite);
        reference_pcd_without_plane_down->RemoveRadiusOutliers(nb_points, search_radius);
        reference_pcd_without_plane_down->RemoveStatisticalOutliers(nb_neighbors, std_ratio);

        auto reference_pcd_without_plane_down_fpfh = open3d::registration::ComputeFPFHFeature(
            *reference_pcd_without_plane_down, open3d::geometry::KDTreeSearchParamHybrid(0.4, 100)); 
        ///////////////////////////////////////////////////////////////////////////////////
        /* double voxel_size = 0.05;

        auto reference_pcd_plane = reference_lidar_frame_->point_cloud_;
        std::shared_ptr<open3d::geometry::PointCloud> pointcloud_ptr_try0(new open3d::geometry::PointCloud);
        *pointcloud_ptr_try0 = reference_pcd_plane;
        auto reference_pcd_plane_down = pointcloud_ptr_try0->VoxelDownSample(voxel_size);
        reference_pcd_plane_down->EstimateNormals(open3d::geometry::KDTreeSearchParamHybrid(0.1, 30));
        auto reference_pcd_plane_down_fpfh = open3d::registration::ComputeFPFHFeature(*reference_pcd_plane_down, open3d::geometry::KDTreeSearchParamHybrid(0.25, 100));

        auto current_pcd_plane = current_lidar_frame_->point_cloud_;
        std::shared_ptr<open3d::geometry::PointCloud> pointcloud_ptr_try1(new open3d::geometry::PointCloud);
        *pointcloud_ptr_try1 = current_pcd_plane;
        auto current_pcd_plane_down = pointcloud_ptr_try1->VoxelDownSample(voxel_size);
        current_pcd_plane_down->EstimateNormals(open3d::geometry::KDTreeSearchParamHybrid(0.1, 30));
        auto current_pcd_plane_down_fpfh = open3d::registration::ComputeFPFHFeature(*current_pcd_plane_down, open3d::geometry::KDTreeSearchParamHybrid(0.25, 100));
 */
        ///////////////////////////////////////////////////////////////////////////////

        std::vector<std::reference_wrapper<const open3d::registration::CorrespondenceChecker>>
        ...
        auto correspondence_checker_normal =open3d::registration::CorrespondenceCheckerBasedOnNormal(0.52359878);
        ...
        auto result_ransac =
            open3d::registration::RegistrationRANSACBasedOnFeatureMatching(
                *current_pcd_plane_down, *reference_pcd_plane_down, *current_pcd_plane_down_fpfh, *reference_pcd_plane_down_fpfh, 0.075,
        ...
        auto result_icp = open3d::registration::RegistrationICP(*current_pcd_without_plane_down, *reference_pcd_without_plane_down, 0.3, result_ransac.transformation_, open3d::registration::TransformationEstimationPointToPlane());
        ...
        *source_transformed_ptr = *current_pcd_plane_down;
        *target_ptr = *reference_pcd_plane_down; */
        ...
        //open3d::visualization::DrawGeometries({current_pcd_plane_down, reference_pcd_plane_down},"view temp");
