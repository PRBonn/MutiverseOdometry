...
/**
 * @brief The LidarOdometerFrameToFrame class declares the Lidar Odometry based
 * on ICP. The type and all parameters can be defined via config file
 */
class LidarOdometerFrameToFrame : public ILidarOdometer {
 public:
  /**
   * @brief Constructor
   * @param initial pose of LidarOdometerFrameToFrame
   * @param config file with the type of icp point to point, point to plane
   * specified with parameters (e.g. kernels) in config file
   */
  LidarOdometerFrameToFrame(const Transform3D &initial_pose,
