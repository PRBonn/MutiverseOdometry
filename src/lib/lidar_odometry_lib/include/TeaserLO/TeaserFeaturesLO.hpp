...
namespace vlo {
/**
 * @brief The TeaserFeaturesLO class declares the Lidar Odometry based
 * on ICP. The type and all parameters can be defined via config file
 */
class TeaserFeaturesLO : public ILidarOdometer {
 public:
  /**
   * @brief Constructor
   * @param initial pose of TeaserFeaturesLO
   * @param config file with the type of icp point to point, point to plane
   * specified with parameters (e.g. kernels) in config file
   */
  TeaserFeaturesLO(const Transform3D &initial_pose);
  std::vector<std::pair<int, int>> findCorrespondencePointsBasedOnFeatures(
